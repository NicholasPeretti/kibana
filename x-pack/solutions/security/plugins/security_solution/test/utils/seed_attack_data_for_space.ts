...
export const seedAttackDataForSpace = async ({
  kbnClient,
  spaceId,
}: {
  kbnClient: KbnClient;
  ...
  const findSeededResponse = await kbnClient.request({
    method: 'GET',
    path: `/s/${spaceId}/api/attack_discovery/_find`,
    query: {
      per_page: 1,
  ...
  await kbnClient.request({
    method: 'POST',
    path: `/s/${spaceId}/internal/elastic_assistant/data_generator/attack_discoveries/_create`,
    headers: {
      'kbn-xsrf': 'true',
...
export const seedAttackScheduleForSpace = async ({
  kbnClient,
  spaceId,
}: {
  kbnClient: KbnClient;
  ...
  const findSchedulesResponse = await kbnClient.request({
    method: 'GET',
    path: `/s/${spaceId}/api/attack_discovery/schedules/_find`,
    query: {
      page: 1,
      per_page: 100,
    },
  ...
  await kbnClient.request({
    method: 'POST',
    path: `/s/${spaceId}/api/attack_discovery/schedules`,
    headers: {
      'kbn-xsrf': 'true',
